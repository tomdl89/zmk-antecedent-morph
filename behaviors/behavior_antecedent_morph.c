...
struct behavior_antecedent_morph_config {
    ...
    uint32_t max_delay_ms; // maximum delay between key release and successive key press for the
                           // adaptive behavior
...
struct behavior_antecedent_morph_data {
    struct zmk_behavior_binding
        *pressed_binding; // the actual behavior that was pressed by the adaptive behavior
};

// data shared by all instances
static int32_t
    code_pressed; // most recently pressed key code (with implicit mods, usage page and keycode)
...
//
// Note that the event structure gives us the keycode (16 bit), the usage page (8 bit) and the
// implicit modifiers (8 bit), but not the explicit modifiers. If the keymap contains the binding
// "&kp RA(Y)", for example, then right-alt is an implicit modifier so that instead of the Y, the
// special character Ü is sent (US International layout).
//
// Whether the user is holding down a shift key at that moment, however, i.e. the explicit
// modifiers, is not known. We could reconstruct this information by tracking the press and release
// events of the modifier keys (keycodes higher than 0xe0) though, but in the present version, the
// potential antecedents are recorded without modifiers.
//
// We here record all key release events of non-modifier keys (keycodes less than 0xe0).
//
// If someone somewhere triggers a key down event with an illegal key code (beyond 0xff), this key
// code is recorded as a potential antecedent, but is then discarded. This way, it is possible to
// trigger 'silent antecedents', e.g. in order to create new dead keys.
...
// is among the configured antecedents and whether the corresponding key release event was no more
...
static int on_antecedent_morph_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    if ((morph >= 0) && delay_ms < cfg->max_delay_ms) {
        ...
        // is among the recorded antecedents, issue the behavior among the 'bindings' that is at the
        // corresponding position to the antecedent among the 'antecedents'.
        //
        // Note that should one of the arrays 'bindings' or 'defaults' are too short, an error is
        // triggered and the behavior never pressed.
...
// The release of the antecedent morph behavior considers the behavior that was recorded in the
// instance data and releases it.
...
static int behavior_antecedent_morph_init(const struct device *dev) {
    ...
    LOG_DBG("zmk,behavior-antecedent-morph serial no. %d defined with %d defaults, %d bindings and "
            "%d antecedents.",
            cfg->serial, cfg->defaults_len, cfg->bindings_len, cfg->antecedents_len);
    for (int i = 0; i < cfg->antecedents_len; i++) {
